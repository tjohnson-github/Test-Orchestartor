...
class operations:
        ...
        try:
            ...
            for key in d:
                if d[key]['session']['bps_in']['mean']:
                    self.obj = dict()
                    self.obj['timestamp'] = time_uuid.TimeUUID.with_timestamp(time.time())
                    self.obj['key'] = key
                    self.obj['test_id'] = self.test_id
                    self.obj['source'] = self.hostname
                    self.obj['op'] = json_data.get('op')
                    self.obj['name'] = 'jumpnet-{0}'.format(self.endpoint)
                    self.obj['payload'] = json.dumps(json_data.get('payload').get(key))
    ...
    def addOPEvent(self, data):
        ...
        try:
            ...
            self.obj = dict()
            self.obj['timestamp'] = time_uuid.TimeUUID.with_timestamp(time.time())
            self.obj['key'] = json_data.get('session')
            self.obj['test_id'] = self.test_id
            self.obj['source'] = self.hostname
            self.obj['op'] = json_data['op']
            self.obj['name'] = 'jumpnet-{0}'.format(self.endpoint)
            self.obj['payload'] = json.dumps(json_data.get('payload'))
            

            self.db.insert('operations_log', self.obj)
